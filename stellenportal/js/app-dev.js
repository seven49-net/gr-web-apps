      ...
      */
      ...
      const vm = createApp({
        setup() {
          ...
          const hidedepartment = hideDepartment;
          const toggle = ref(false);
          const departmentssearch = ref("search");
          ...
          function getTranslation(
            prop,
            lang,
            fallback = "missing translation for " + prop,
          ) {
            var trans = i18n;
            var out = trans["de"].hasOwnProperty(prop) ? trans["de"][prop] : fallback;
            if (trans.hasOwnProperty(lang) && trans[lang].hasOwnProperty(prop))
              out = trans[lang][prop];
            return out;
          }
          function getNoresultText(prop, lang) {
            console.log("getNoresultText", lang);
            return getTranslation(prop, lang);
          }
          function updateCopyLink() {
            var noResultText = noresulttext.value;
            var hide = vm.toggle;
            console.log(copylink.value);
            return (copylink.value = updateQueryStringParameter(
              updateQueryStringParameter(
                copylink.value,
                "noresulttext",
          ...
          function updateonkeyup() {
            ...
            copylink.value = updateQueryStringParameter(
              link,
              "noresulttext",
              encodeURIComponent(noresulttext.value),
            );
          }
          ...
          const copyLinkToClipord = function () {
